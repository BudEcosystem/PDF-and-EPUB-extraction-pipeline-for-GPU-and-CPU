...
def signal_handler(sig, frame):
    ...
    if current_bookname and current_page_num is not None:
        store_book_progress(
            current_bookname, current_page_num, current_bookId, current_book_number
        )
    exit(0)
...
def process_book(bookname, start_page, bookId):
    ...
    try:
        for page_num in range(start_page, num_pages):
            ...
            print(page_num)
            page_object, page_layout_info = process_page(
                page_num, book_path, book_folder, bookname, bookId
            )
            ...
            if book_document:
                bookdata.update_one(
                    {"_id": book_document["_id"]}, {"$push": {"pages": page_object}}
                )
            else:
                new_book_document = {
                    "bookId": bookId,
            ...
            if book_layout_doc:
                book_layout.update_one(
                    {"_id": book_layout_doc["_id"]},
                    {"$push": {"pages": page_layout_info}},
                )
            else:
                new_book_layout = {
                    "bookId": bookId,
...
def process_page(page_num, book_path, book_folder, bookname, bookId):
    ...
    page_content, page_tables, page_figures, page_equations, layout_blocks = (
        process_image(image_path, page_num, bookname, bookId, pdf_book)
    )
    ...
    page_layout_info = {"id": pageId, "layout_info": layout_blocks}
    print(page_num, "done")
...
def process_image(imagepath, page_num, bookname, bookId, pdf_book):
    ...
    try:
        ...
        if document:
            pdFigCap = True
            layout_blocks = [
                block for block in layout_blocks if block["type"] != "Figure"
            ]
            figures_block = []
        ...
        if not layout_blocks or not any(
            block["type"] in ["Table", "Figure"] for block in layout_blocks
        ):
            try:
                print("extracting using naugat")
                page_content = extract_text_equation_with_nougat(
                    imagepath, page_equations, page_num, bookname, bookId
                )
                return (
                    page_content,
                    page_tables,
                ...
                print(
                    f"An error occurred while processing {bookname}, page {page_num} with nougat: {str(e)}"
                )
                error = {
                    "error": str(e),
                ...
                if document:
                    error_collection.update_one(
                        {"_id": document["_id"]}, {"$push": {"pages": error}}
                    )
                else:
                    new_error_doc = {
                        "bookId": bookId,
        ...
        if (
            not pdFigCap
            and len(layout_blocks) == 1
            and layout_blocks[0]["type"] == "Figure"
        ):
            x1, y1, x2, y2 = (
                layout_blocks[0]["x_1"],
                layout_blocks[0]["y_1"],
            ...
            if image.height > 1500:
                ...
                figure_url = None
                page_figures.append({"id": figureId, "url": figure_url, "caption": ""})
        ...
        print(
            f"An error occurred while processing {bookname}, page {page_num}: {str(e)}, line_numbe {traceback.extract_tb(e.__traceback__)[-1].lineno}"
        )
        error = {
            "error": str(e),
            "page_number": page_num,
...
@timeit
def sort_text_blocks_and_extract_data(
    blocks, imagepath, page_tables, page_figures, page_equations, pdFigCap
):
    """
    ...
    for i, block in enumerate(sorted_blocks):
        ...
        if block["type"] == "Table":
            ...
            if pdFigCap:
                ...
                output = process_publeynet_figure(
                    block, imagepath, prev_block, next_block, output, page_figures
                )
...
def process_figure(figure_block, imagepath, output, page_figures):
    ...
    figure_url = None
    page_figures.append(
        {"id": figureId, "url": figure_url, "caption": figure_block["caption"]}
    )
    if figure_url is not None and os.path.exists(figure_image_path):
        os.remove(figure_image_path)
    return output
...
@timeit
def process_publeynet_figure(
    figure_block, imagepath, prev_block, next_block, output, page_figures
):
    """
    Processes a figure block, extracts the figures and figure captions, and adds it's id to the output (for maintaining sequence).
    ...
    # figure_url=upload_to_aws_s3(figure_image_path, figureId)
    figure_url = None
    page_figures.append({"id": figureId, "url": figure_url, "caption": caption})
    if figure_url is not None and os.path.exists(figure_image_path):
        os.remove(figure_image_path)
    return output
...
def extract_text_equation_with_nougat(
    image_path, page_equations, page_num, bookname, bookId
):
    """
    ...
    def replace_with_uuid(match):
        ...
        page_equations.append(
            {"id": equationId, "text": match_text, "text_to_speech": text_to_speech}
        )
    ...
    print(
        f"Memory Usage for nougat function: {process.memory_info().rss / (1024 ** 2):.2f} MB"
    )
    gpus = GPUtil.getGPUs()
...
def get_latext_text(pdf_path, page_num, bookname, bookId):
    try:
        command = ["nougat", pdf_path, "--no-skipping"]
...
def get_figure_and_captions(book_path, bookname, bookId):
    ...
    process = psutil.Process(os.getpid())
    print(
        f"Memory Usage for figure caption function: {process.memory_info().rss / (1024 ** 2):.2f} MB"
    )
    gpus = GPUtil.getGPUs()
    for i, gpu in enumerate(gpus):
        print(f"GPU {i + 1} - GPU Name: {gpu.name}")
    ...
    with open(book_path, "rb") as pdf_file:
        ...
        for i in range(0, num_pages, pages_per_split):
            ...
            output_filename = os.path.join(
                output_directory, f"output_{i // pages_per_split + 1}.pdf"
            )
    ...
    try:
        ...
        if book_data:
            figure_caption.insert_one(
                {"bookId": bookId, "book": bookname, "pages": book_data}
            )
        ...
        if os.path.exists(book_output):
            shutil.rmtree(book_output)
        print(
            f"Unable to get figure and figure caption for this {bookname}, {str(e)}, line_number {traceback.extract_tb(e.__traceback__)[-1].lineno}"
        )
        return []
