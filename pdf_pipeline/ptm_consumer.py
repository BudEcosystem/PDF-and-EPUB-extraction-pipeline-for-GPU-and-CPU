...
publaynet_model = LayoutAnalysisPipeline.from_pretrained(pretrained_model_name="publaynet", device=device)
tablebank_model = LayoutAnalysisPipeline.from_pretrained(pretrained_model_name="tablebank", device=device)
mathformuladetection_model = LayoutAnalysisPipeline.from_pretrained(pretrained_model_name="mfd", device=device)
...
def ptm_layout(ch, method, properties, body):
    ...
    try:
        ...
        for img_path, page_num, layout_blocks in get_layout_docxtract(image_paths, page_nums):
            queue_msg = {
                "bookId": bookId,
                "page_num": page_num
            }
...
def get_layout_docxtract(image_paths, page_nums):
    ...
    publaynet_layouts = [each._blocks for each in pub_layout]
    images = [Image.open(img_path) for img_path in image_paths]
    tb_layout = tablebank_model(images=images)
    tablebank_layouts = [each._blocks for each in tb_layout]
    images = [Image.open(img_path) for img_path in image_paths]
    mfd_layout = mathformuladetection_model(images=images)
    mathformuladetection_layouts = [each._blocks for each in mfd_layout]
